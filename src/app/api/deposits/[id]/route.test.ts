...
// See `src/app/api/members/[id]/route.test.ts` for the rationale on mocking
...
const PREFIX = "deposits-id-route-test-";
...
function patchRequest(body: unknown) {
  return new Request("http://localhost/api/deposits/whatever", {
    method: "PATCH",
...
describe("/api/deposits/[id] route handlers", () => {
  let adminId: string;
  let memberId: string;
  ...
  beforeAll(async () => {
    ...
    const deposit = await prisma.monthlyDeposit.create({
      data: {
  ...
  describe("PATCH", () => {
    it("returns 401 when there is no session", async () => {
      ...
      const res = await PATCH(patchRequest({ amount: 500 }), paramsFor(depositId));
    ...
    it("returns 404 for an unknown deposit id", async () => {
    ...
    it("returns 400 for invalid input (negative amount)", async () => {
      ...
      const res = await PATCH(patchRequest({ amount: -50 }), paramsFor(depositId));

      expect(res.status).toBe(400);
    });

    it("returns 200, updates the deposit, and writes an ActivityLog UPDATE row for a valid ADMIN request", async () => {
      mockedAuth.mockResolvedValue(sessionFor("ADMIN", adminId));
      ...
      const log = await prisma.activityLog.findFirst({
        where: { entityType: "MonthlyDeposit", entityId: depositId, action: "UPDATE" },
